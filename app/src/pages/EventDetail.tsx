...
import {
  ...
  CheckCircle, AlertCircle, ArrowRight, Users, Loader2
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    getEvent(id)
      .then((ev) => {
        setEvent(ev);
        if (profile) {
          return isRegistered(profile.$id, id).then(setAlreadyRegistered);
        }
      })
  ...
  if (error || !event) return (
    <div className="flex justify-center items-center min-h-screen">
      <div className="text-center">
        ...
        <p className="text-cyber-gray mb-4">{error || 'Event not found'}</p>
        <Link to="/events" className="text-cyber-cyan hover:underline">Browse Events</Link>
      </div>
    </div>
  );

  // Parse JSON fields from event_details
  const prizes: { place: string; amount: string; description: string }[] =
    (() => { try { return JSON.parse(event.prizes || '[]'); } catch { return []; } })();
  const rules: string[] =
    (() => { try { return JSON.parse(event.rules || '[]'); } catch { return []; } })();
  const eligibility: string[] =
    (() => { try { return JSON.parse(event.eligibility || '[]'); } catch { return []; } })();
  const judgeIds: string[] =
    (() => { try { return JSON.parse(event.judgeIds || '[]'); } catch { return []; } })();
    ...
    <div className="min-h-screen">
      ...
      <div className="relative h-64 sm:h-80 overflow-hidden">
        ...
        <div className="absolute top-6 left-6">
          <Link to="/events"
            className="flex items-center gap-1 text-white/80 hover:text-cyber-cyan transition-colors text-sm">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {prizes.length > 0 && (
              <div className="cyber-card p-6">
                <h2 className="text-xl font-heading font-semibold text-white mb-4">Prizes</h2>
                ...
                <ul className="space-y-2">
                  {rules.map((rule, i) => (
                    <li key={i} className="flex items-start gap-3 text-cyber-gray">
                      <CheckCircle className="w-4 h-4 text-cyber-cyan mt-0.5 flex-shrink-0" />{rule}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div>
            <div className="cyber-card p-6 sticky top-24 space-y-5">
              <div>
                ...
                {regDeadlinePassed && (
                  <p className="text-red-400 text-xs mt-1">Registration has closed</p>
                )}
              </div>
              ...
              {alreadyRegistered ? (
                <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg text-center">
                  ...
                  <Link to="/participant/my-events"
                    className="text-cyber-cyan text-sm hover:underline mt-1 inline-block">
                    View in My Events →
                  </Link>
                </div>
              ) : canRegister ? (
